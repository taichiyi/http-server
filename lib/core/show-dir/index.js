  ...
  document.addEventListener('click',async(event)=>{
    ...
    if(target.nodeName==="BUTTON"){
      ...
      if(target.className==="delete"){
        ...
        if(result){
          await fetch("https://api-dev.taichiyi.com/tcyOss/delete", {
            method: 'post',
            body: JSON.stringify({
              dir: dataset.dir,
        ...
        if(nextName!==null){
          await fetch("https://api-dev.taichiyi.com/tcyOss/rename", {
            method: 'post',
            body: JSON.stringify({
              dir: dataset.dir,
...
const makeDirectoryHTML = (dir) => {
  return `
  <div>
    上传文件 <input type="file" class="upload-file" data-dir="${dir}" />
  </div>
  `;
};
  ...
  document.addEventListener('change',async(event)=>{
    ...
    if(target.nodeName==="INPUT"){
      ...
      if(target.className==="upload-file"){
        ...
        await fetch("https://api-dev.taichiyi.com/tcyOss/upload", {
          method: 'post',
          body: formData,
          credentials: 'include',
        });
        ...
        function render(dirs, renderFiles, errs) {
          ...
          const writeRow = (file) => {
            ...
            html +=
              ...
              `<td class="operation">
                <button class="delete" data-dir="${dir}" data-displayName="${displayNameText}">删除</button>
